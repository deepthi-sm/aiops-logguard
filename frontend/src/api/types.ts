/**
 * TypeScript mirrors of the Pydantic models in backend/api/schemas.py.
 *
 * IMPORTANT: keep these in sync with the backend. The backend's `schemas.py`
 * is the source of truth (CLAUDE.md "How Claude Code should work in this
 * repo"). When the API contract changes, update both files together.
 *
 * A future PR can replace this hand-written file with a codegen step that
 * reads /openapi.json and emits these types automatically.
 */

export type Severity = "critical" | "warning" | "info";
export type ExplanationStatus = "pending" | "ready" | "failed";
export type Feedback = "true_positive" | "false_positive";
export type TimelineWindow = "1h" | "24h" | "7d";
export type DriftLevel = "healthy" | "drift_high" | "drift_critical";

export interface ContributingLine {
  line: string;
  attention: number; // 0..1
}

export interface Anomaly {
  id: string;
  detected_at: string; // ISO 8601 UTC with 'Z' suffix
  severity: Severity;
  source: string;
  ensemble_score: number; // 0..1
  confidence: number; // 0..1
  failure_probability: number; // 0..1
  predicted_failure_window_min: number | null;
  log_template: string;
  sequence_preview: string[];
  top_contributing_lines: ContributingLine[];
  explanation_status: ExplanationStatus;
  cluster_id: string;
  cluster_size: number;
}

export interface AnomalyListResponse {
  items: Anomaly[];
  next_cursor: string | null;
}

export interface SimilarIncident {
  incident_id: string;
  template: string;
  resolved_at: string | null;
  similarity_score: number; // 0..1
}

export interface Explanation {
  root_cause: string;
  recommended_fix: string;
  similar_incidents: SimilarIncident[];
  attention_weights: number[];
}

export interface MetricsSummary {
  total_24h: number;
  critical_24h: number;
  warning_24h: number;
  info_24h: number;
  avg_confidence: number; // 0..1
  drift_score: number;
  last_retrain: string | null;
}

export interface TimelineBucket {
  ts: string; // ISO 8601 UTC
  critical: number;
  warning: number;
  info: number;
}

export interface TimelineResponse {
  window: TimelineWindow;
  buckets: TimelineBucket[];
}

export interface DriftStatus {
  drift_score: number;
  last_retrain: string | null;
  status: DriftLevel;
  psi_score: number;
}

export interface FeedbackRequest {
  feedback: Feedback;
}

export interface FeedbackResponse {
  ok: boolean;
}

export interface HealthResponse {
  status: "ok" | "degraded";
  version: string;
  uptime_s: number;
}

// -- WebSocket envelopes --------------------------------------------------

export type WsMessage =
  | { type: "anomaly"; data: Anomaly }
  | {
      type: "explanation_ready";
      data: { anomaly_id: string; explanation_status: ExplanationStatus };
    }
  | { type: "ping" };

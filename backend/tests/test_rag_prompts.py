...
def test_system_prompt_pins_two_section_format():
    """The parser depends on these section headers — if the system
    prompt drifts, parsing silently breaks. Pin the contract here."""
...
class TestBuildUserPrompt:
    ...
    def test_includes_similar_incidents_in_order(self):
        ...
        # Both incidents appear with their similarity.
        assert "syn_001" in out
        assert "syn_002" in out
        # syn_001 (rank 1) appears before syn_002 (rank 2).
        assert out.index("syn_001") < out.index("syn_002")
        # Each renders root_cause + recommended_fix.
        assert "cause for syn_001" in out
        assert "fix for syn_001" in out

    def test_zero_hits_explicitly_documented(self):
        """When FAISS returns nothing, the prompt should tell LLaMA to
        ground in the alert facts alone — silence is misleading."""
        out = build_user_prompt(
            log_template="t",
            sequence_preview=[],
            source="h",
        ...
        assert "No similar prior incidents" in out
